...
resourcegroup_name = 'sk'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'sk'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
coffee.flavor = 'Pumpkinspice'
coffee.size = 'Medium'
coffee.price = 4.99
table_service.insert_entity('itemstable', coffee)
print('Created entry for Alterra...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cars'", select='make,price')
for item in items:
    print('Name: ' + item.make)
    print('Cost: ' + str(item.price) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffees'", select='brand,price')
for item in items:
